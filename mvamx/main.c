...
int main(int argc, char **argv)
{
	...
	if (X == NULL) {
		fprintf(stderr, "Error: Failed to allocate memory for X array\n");
		freemodel(qn);
		return -1;
	}
	...
	for (m = 0; m < qn->M; m++) {
		...
		if (Q[m] == NULL) {
			fprintf(stderr, "Error: Failed to allocate memory for Q[%d]\n", m);
			for (r = 0; r < qn->R; r++) mpq_clear(X[r]);
			for (int i = 0; i < m; i++) {
				for (r = 0; r < qn->R; r++) mpq_clear(Q[i][r]);
				free(Q[i]);
			}
			free(X); free(Q);
			freemodel(qn);
	...
	mpq_t G;
	mpq_init(G);
	double logG = mvamx(qn, X, Q, G);

	if (logG < 0 && mpq_cmp_ui(G, 0, 1) == 0) {
		/* solver failed */
		for (r = 0; r < qn->R; r++) mpq_clear(X[r]);
		for (m = 0; m < qn->M; m++) {
			for (r = 0; r < qn->R; r++) mpq_clear(Q[m][r]);
			free(Q[m]);
		}
		free(Q); free(X);
		mpq_clear(G);
	...
	if (log_output) {
		...
		mpf_t fval; mpf_init(fval); mpf_set_q(fval, G);
		printf("%.15e\n", mpf_get_d(fval));
		mpf_clear(fval);
	} else if (throughput_output) {
		for (r = 0; r < qn->R; r++) {
			mpf_t xval; mpf_init(xval); mpf_set_q(xval, X[r]);
			printf("%.15e\n", mpf_get_d(xval));
			mpf_clear(xval);
		}
		...
		for (m = 0; m < qn->M; m++) {
			for (r = 0; r < qn->R; r++) {
				mpf_t qval; mpf_init(qval); mpf_set_q(qval, Q[m][r]);
		...
		printf("Normalizing constant (closed sub-model):\n");
		gmp_printf("G = %Qd\n", G);
		printf("log(G) = %.15e\n", logG);
		...
		printf("=========================================\n");
		t1 = CPUTIME;
		printf("Elapsed time (MVA-MX): %.6f s\n", t1 - t0);
	} else {
		/* Default: pretty-print as doubles */
